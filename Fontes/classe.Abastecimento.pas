...
begin
  //Função para consulta( neste caso, utilizada somente no relatório)
  try
    ...
    begin
      close;
      sql.Clear;
      SQL.Add('select codigo, iif(bomba = 1,''Bomba 2'',''Bomba 1'')bomba, data,qtde, iif(tanque = 1, ''Óleo Diesel'',''Gasolina'')TANQUE, valor from abastecimento where data between :p_texto and :p_texto2');
...
var
  QryInserir: TFDQuery;
begin
  //Função para inserir ou alterar (caso fosse necessário)
  try
    try
      FConexao.Connected:= false;
      FConexao.Connected:= true;
